...
# Generate the lock file for a workflow that uses fuzzy scheduling
gh aw compile [workflow-name] --schedule-seed dotnet/vitals
...
gh aw compile .github/workflows/<name>.md --schedule-seed dotnet/vitals

# Recompile all workflows
gh aw compile --schedule-seed dotnet/vitals
```
...
The PAT pool blocks (`on.steps`, `jobs.pre-activation`, `engine`) naturally fall into the **Execution** group at the bottom. Keep them together as the last items in the frontmatter.

### Separating PAT pool boilerplate from workflow-specific content

When workflow-specific frontmatter must coexist within a key that also contains PAT pool content (e.g., the workflow has its own `on:` triggers alongside the PAT pool `steps`), use these conventions:

- Place the workflow's own configuration **first** within the shared key (e.g., trigger filters under `on:`).
- Insert the PAT pool comment block (shown below) as a visual separator before the PAT pool entries.
- If the workflow defines its own `jobs:` or `engine:` settings, place them **before** the PAT pool entries within those keys and use a blank comment line (`#`) to separate custom content from PAT pool boilerplate.

This ensures readers can quickly distinguish between what's unique to the workflow and what's shared boilerplate.
...
When **creating** or **updating** an agentic workflow, always include the following blocks in the **Execution** section of the frontmatter (at the bottom). These go between the `---` markers alongside the workflow's other frontmatter.

### Comment block (placed before the PAT pool steps, inside the `on:` block)

Always include this comment block before the PAT pool steps to visually separate them from the workflow's own trigger configuration:
...
Add this under the `on:` key, at the same level as the trigger configuration, immediately after the comment block above. Place the workflow's own trigger configuration (e.g., event filters) **before** the comment block so that custom `on:` content appears first.
...
If the workflow has its own `jobs:` entries, place them before this block and separate with a blank comment line.
...
This block **replaces** any simple `engine: copilot` declaration. If the workflow needs additional engine configuration, include it in this object form alongside the PAT override.
