...
This reference lists the MCP specification repository, all official MCP SDK repositories, and the themes to search for when cross-referencing issues.

## MCP Specification Repository

| Repository | Description |
|---|---|
| [modelcontextprotocol/modelcontextprotocol](https://github.com/modelcontextprotocol/modelcontextprotocol) | MCP specification, SEPs (Specification Enhancement Proposals), and governance |

PRs labeled `SEP` in this repository are Specification Enhancement Proposals tracking protocol changes.
...
When identifying common themes across SDK repos, search open issues for these themes. Use the keyword patterns to match against issue titles and the first 500 characters of issue bodies.
...
For each theme:
1. Search open issues in each SDK repo using the keyword patterns
2. Track which themes are prevalent in each SDK
3. Build the Common Themes matrix and per-SDK evidence links
